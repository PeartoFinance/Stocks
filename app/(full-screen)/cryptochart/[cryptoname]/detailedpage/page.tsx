...
export default function CryptoDetailedTradingPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const response: any = await cryptoService.getCoinDetails(cryptoname);
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      try {
        ...
        const interval = selectedPeriod === '1D' ? '1d' : '1d';
        
        const response = await cryptoService.getHistory(cryptoname, mappedPeriod, interval);
    ...
    fetchChartData();
  }, [cryptoname, selectedPeriod, selectedChartType]);
  ...
  const priceChange = cryptoData?.change || 0;
  const priceChangePercent = cryptoData?.changePercent || 0;
