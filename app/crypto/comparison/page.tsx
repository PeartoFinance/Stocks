      ...
      <main className="p-4 lg:p-6">
        <div className="max-w-[1600px] mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-3 sm:mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-[#fafafa] mb-1">Crypto Comparison</h1>
            <p className="text-xs sm:text-sm text-slate-600 dark:text-[#a3a3a3]">Compare up to 5 cryptocurrencies side by side</p>
          </motion.div>

          {/* Search Section */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-[#171717] rounded-lg p-3 sm:p-4 border border-slate-200 dark:border-[#262626] mb-4 sm:mb-6">
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <h2 className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-[#fafafa]">Add Cryptocurrencies</h2>
            ...
            <div className="relative mb-2 sm:mb-3">
              ...
              <input
                ...
                className="w-full pl-8 sm:pl-10 pr-4 py-2 text-xs sm:text-sm border border-slate-300 dark:border-[#262626] rounded-lg bg-white dark:bg-[#0a0a0a] text-slate-900 dark:text-[#fafafa] focus:ring-2 focus:ring-blue-500 dark:focus:ring-emerald-500"
              />
            ...
            {searchResults.length > 0 && (
              <div className="border border-slate-200 dark:border-[#262626] rounded-lg max-h-48 sm:max-h-64 overflow-y-auto">
                {searchResults.map((crypto) => (
                  <div key={crypto.symbol} className="flex items-center justify-between p-2 sm:p-3 hover:bg-slate-50 dark:hover:bg-[#1a1a1a] border-b border-gray-100 dark:border-[#262626] last:border-b-0">
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-[#fafafa] truncate">{crypto.symbol}</h4>
                      <p className="text-[10px] sm:text-xs text-slate-600 dark:text-[#a3a3a3] truncate">{crypto.name}</p>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className="text-right">
                        <p className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-[#fafafa]">${crypto.price?.toFixed(2) || '0.00'}</p>
            ...
            {searchTerm === '' && comparedCryptos.length < 5 && (
              <div>
                <h4 className="text-[10px] sm:text-xs font-medium text-slate-700 dark:text-[#a3a3a3] mb-2">Popular Cryptocurrencies</h4>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'ADA'].map((symbol) => (
                    <button
                      ...
                      className="px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-medium bg-slate-100 dark:bg-[#1a1a1a] hover:bg-gray-200 dark:hover:bg-[#262626] disabled:opacity-50 text-slate-700 dark:text-[#fafafa] rounded-md"
                    >
                      {symbol}
                    </button>
                  ))}
          ...
          {comparedCryptos.length > 0 && (
            <>
              <div className="bg-white dark:bg-[#171717] rounded-lg p-1 border border-slate-200 dark:border-[#262626] mb-4 sm:mb-6">
