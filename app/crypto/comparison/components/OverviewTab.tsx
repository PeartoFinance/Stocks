  ...
  if (comparedCryptos.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-600 dark:text-[#a3a3a3]">Add cryptocurrencies to see the overview</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {comparedCryptos.map((crypto) => {
          ...
          return (
            <div key={crypto.symbol} className={`bg-white dark:bg-[#171717] rounded-lg p-4 border-2 ${isPositive ? 'border-green-200 dark:border-green-500/30' : 'border-red-200 dark:border-red-500/30'}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-lg font-bold text-gray-900 dark:text-[#fafafa]">{crypto.symbol}</h4>
                  <p className="text-xs text-gray-600 dark:text-[#a3a3a3] truncate">{crypto.name}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-gray-900 dark:text-[#fafafa]">${crypto.price.toFixed(2)}</span>
                </div>
                <div className={`flex justify-between items-center p-2 rounded ${isPositive ? 'bg-green-50 dark:bg-green-500/10' : 'bg-red-50 dark:bg-red-500/10'}`}>
                  <span className="text-sm text-gray-700 dark:text-[#a3a3a3]">24h Change</span>
                ...
                <div className="pt-2 border-t border-gray-200 dark:border-[#262626] space-y-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium text-gray-900 dark:text-[#fafafa]">{formatLargeNumber(crypto.marketCap)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600 dark:text-[#a3a3a3]">24h Volume</span>
                    <span className="font-medium text-gray-900 dark:text-[#fafafa]">{formatLargeNumber(crypto.volume)}</span>
                  </div>
                  {crypto.rank && (
                    <div className="flex justify-between text-xs">
                      <span className="text-gray-600 dark:text-[#a3a3a3]">Rank</span>
                      <span className="font-medium text-gray-900 dark:text-[#fafafa]">#{crypto.rank}</span>
                    </div>
                  )}
                </div>
              </div>
