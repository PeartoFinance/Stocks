  ...
  if (comparedCryptos.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Add cryptocurrencies to see the overview</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {comparedCryptos.map((crypto) => {
          ...
          return (
            <div key={crypto.symbol} className={`bg-white dark:bg-gray-800 rounded-lg p-4 border-2 ${isPositive ? 'border-green-200 dark:border-green-500/30' : 'border-red-200 dark:border-red-500/30'}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-lg font-bold text-gray-900 dark:text-white">{crypto.symbol}</h4>
                  <p className="text-xs text-gray-600 dark:text-gray-400 truncate">{crypto.name}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-gray-900 dark:text-white">${crypto.price.toFixed(2)}</span>
                </div>
                <div className={`flex justify-between items-center p-2 rounded ${isPositive ? 'bg-green-50 dark:bg-green-500/10' : 'bg-red-50 dark:bg-red-500/10'}`}>
                  <span className="text-sm text-gray-700 dark:text-gray-400">24h Change</span>
