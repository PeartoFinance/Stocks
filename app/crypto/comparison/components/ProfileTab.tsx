  ...
  if (comparedCryptos.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No cryptos selected</h3>
        <p className="text-sm text-gray-600 dark:text-gray-400">Add cryptocurrencies to see profiles</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {comparedCryptos.map((crypto) => {
        ...
        return (
          <div key={crypto.symbol} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            {/* Header */}
            <div className={`p-3 ${isPositive ? 'bg-gradient-to-r from-green-50 to-green-100 dark:from-green-500/10 dark:to-green-500/5' : 'bg-gradient-to-r from-red-50 to-red-100 dark:from-red-500/10 dark:to-red-500/5'}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-gray-900 dark:text-white">{crypto.symbol}</h3>
                    <p className="text-[10px] text-gray-600 dark:text-gray-400">{crypto.name}</p>
                  </div>
                </div>
                {crypto.rank && (
                  <div className="bg-white dark:bg-gray-800 px-2 py-0.5 rounded-full border border-gray-200 dark:border-gray-700">
                    <span className="text-[9px] font-bold text-gray-700 dark:text-white">#{crypto.rank}</span>
                  </div>
                )}
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-bold text-gray-900 dark:text-white">${crypto.price.toFixed(2)}</span>
            ...
            <div className="p-3 space-y-3">
              {/* Market Data */}
              <div>
                <h4 className="text-[10px] font-bold text-gray-900 dark:text-white mb-1.5 flex items-center gap-1">
                  <BarChart3 className="h-3 w-3 text-blue-600 dark:text-emerald-500" />
                  Market
                </h4>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-gray-50 dark:bg-gray-700 p-2 rounded">
                    <p className="text-[9px] text-gray-600 dark:text-gray-400 mb-0.5">Market Cap</p>
                    <p className="text-xs font-bold text-gray-900 dark:text-white">{formatLargeNumber(crypto.marketCap)}</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-700 p-2 rounded">
                    <p className="text-[9px] text-gray-600 dark:text-gray-400 mb-0.5">24h Volume</p>
                    <p className="text-xs font-bold text-gray-900 dark:text-white">{formatLargeNumber(crypto.volume)}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {crypto.low24h && (
                      <div className="bg-red-50 dark:bg-red-500/10 p-2 rounded border border-red-200 dark:border-red-500/20">
                        ...
                        <p className="text-xs font-bold text-red-900 dark:text-white">${crypto.low24h.toFixed(2)}</p>
                      </div>
                    )}
                    {crypto.high24h && (
                      <div className="bg-green-50 dark:bg-green-500/10 p-2 rounded border border-green-200 dark:border-green-500/20">
                        ...
                        <p className="text-xs font-bold text-green-900 dark:text-white">${crypto.high24h.toFixed(2)}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {crypto.circulatingSupply && (
                      <div className="flex justify-between items-center p-1.5 bg-gray-50 dark:bg-gray-700 rounded">
                        <span className="text-[9px] text-gray-600 dark:text-gray-400">Circulating</span>
                    ...
                    {crypto.maxSupply && (
                      <div className="flex justify-between items-center p-1.5 bg-gray-50 dark:bg-gray-700 rounded">
                        <span className="text-[9px] text-gray-600 dark:text-gray-400">Max</span>
                        <span className="text-[10px] font-bold text-gray-900 dark:text-white">
