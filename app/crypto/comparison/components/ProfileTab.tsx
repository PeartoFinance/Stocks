  ...
  if (comparedCryptos.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-400">Add cryptocurrencies to see profiles</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {comparedCryptos.map((crypto) => {
        ...
        return (
          <div key={crypto.symbol} className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
            {/* Header */}
            <div className={`p-3 ${isPositive ? 'bg-gradient-to-r from-green-50 to-green-100 dark:from-green-500/10 dark:to-green-500/5' : 'bg-gradient-to-r from-red-50 to-red-100 dark:from-red-500/10 dark:to-red-500/5'}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base font-medium text-gray-900 dark:text-white">{crypto.symbol}</h3>
                    <p className="text-[10px] text-gray-600 dark:text-slate-400">{crypto.name}</p>
                  </div>
                </div>
                {crypto.rank && (
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-medium text-gray-900 dark:text-white">${crypto.price.toFixed(2)}</span>
            ...
            <div className="p-3 space-y-3">
              {/* Market Data */}
              <div>
                <h4 className="text-[10px] font-medium text-gray-900 dark:text-white mb-1.5 flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-50 dark:bg-slate-700 p-2 rounded">
                    <p className="text-[9px] text-gray-600 dark:text-slate-400 mb-0.5">Market Cap</p>
                    <p className="text-xs font-medium text-gray-900 dark:text-white">{formatLargeNumber(crypto.marketCap)}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700 p-2 rounded">
                    <p className="text-[9px] text-gray-600 dark:text-slate-400 mb-0.5">24h Volume</p>
                    <p className="text-xs font-medium text-gray-900 dark:text-white">{formatLargeNumber(crypto.volume)}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-1">
                    {crypto.circulatingSupply && (
                      <div className="flex justify-between items-center p-1.5 bg-slate-50 dark:bg-slate-700 rounded">
                        <span className="text-[9px] text-gray-600 dark:text-slate-400">Circulating</span>
                    ...
                    {crypto.maxSupply && (
                      <div className="flex justify-between items-center p-1.5 bg-slate-50 dark:bg-slate-700 rounded">
                        <span className="text-[9px] text-gray-600 dark:text-slate-400">Max</span>
                        <span className="text-[10px] font-medium text-gray-900 dark:text-white">
