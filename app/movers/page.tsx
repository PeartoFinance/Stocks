      ...
      <div className="flex flex-col xl:flex-row gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="flex-1">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-gray-600">Top gainers, losers, and highest volume across the market.</p>
          </motion.div>

          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100 mb-4 sm:mb-6">
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 sm:items-center sm:justify-between">
              <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                {(['gainers', 'losers', 'volume'] as const).map(t => (
                  <button key={t} onClick={() => setTab(t)} className={`px-3 py-1.5 rounded-full text-xs sm:text-sm whitespace-nowrap ${tab === t ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{t}</button>
                ))}
              </div>
              <div className="relative w-full sm:w-auto sm:min-w-[280px]">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-gray-400" />
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search symbol or name…" className="w-full pl-10 pr-4 py-2 text-sm sm:text-base border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
            </div>
          </div>

          {loading ? (
            <div className="h-60 sm:h-80 flex items-center justify-center text-gray-500"><Activity className="h-5 w-5 sm:h-6 sm:w-6 animate-spin mr-2" />Loading movers…</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {filtered.slice(0, 8).map((m, i) => (
                <Link key={m.id} href={`/stock/${m.symbol.toLowerCase()}`}>
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100 hover:shadow-xl cursor-pointer">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <h3 className="text-base sm:text-lg font-bold text-gray-900">{m.symbol}</h3>
                        <p className="text-xs sm:text-sm text-gray-600 truncate">{m.name}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">{formatPrice(m.price)}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
                      <div className="p-2 sm:p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-sm sm:text-base font-semibold text-gray-900">{formatNumber(m.volume)}</p>
                      </div>
                      <div className="p-2 sm:p-3 bg-gray-50 rounded-lg">
                        <p className="text-xs text-gray-500">Intraday Range</p>
                        <p className="text-sm sm:text-base font-semibold text-gray-900">High bias</p>
                      </div>
                    </div>
                  </motion.div>
                </Link>
