  ...
  if (comparedStocks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">Add stocks to see profiles</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {comparedStocks.map((stock) => {
        ...
        return (
          <div key={stock.symbol} className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
            {/* Header */}
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Link href={`/stock/${stock.symbol.toLowerCase()}`} className="text-xl font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-emerald-500">
                    {stock.symbol}
                  </Link>
            ...
            <div className="space-y-4 mb-6">
              <div className="flex items-start gap-3">
                <Building2 className="h-5 w-5 text-gray-400 dark:text-gray-400 mt-0.5" />
                <div className="flex-1">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Sector & Industry</p>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{stock.sector || 'N/A'}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">{stock.industry || 'N/A'}</p>
                </div>
              </div>

              <div className="flex items-start gap-3">
                <Globe className="h-5 w-5 text-gray-400 dark:text-gray-400 mt-0.5" />
                <div className="flex-1">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Exchange</p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  ...
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{formatLargeNumber(stock.marketCap)}</p>
                </div>

                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">P/E Ratio</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{stock.peRatio?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">EPS</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">${stock.eps?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Beta</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{stock.beta?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Volume</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{formatLargeNumber(stock.volume)}</p>
                </div>

                <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Div Yield</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{stock.dividendYield?.toFixed(2) || 'N/A'}%</p>
                </div>
              </div>

              {/* 52 Week Range */}
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">52 Week Range</p>
