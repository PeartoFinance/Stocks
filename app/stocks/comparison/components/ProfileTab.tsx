          ...
          <div key={stock.symbol} className="bg-white dark:bg-slate-900/95 rounded-lg p-6 border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Link href={`/stock/${stock.symbol.toLowerCase()}`} className="text-xl font-bold text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-emerald-500">
                    {stock.symbol}
                  </Link>
            ...
            <div className="space-y-4 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Sector & Industry</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{stock.sector || 'N/A'}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{stock.industry || 'N/A'}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{stock.exchange || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{formatLargeNumber(stock.marketCap)}</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">P/E Ratio</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{stock.peRatio?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">EPS</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">${stock.eps?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Beta</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{stock.beta?.toFixed(2) || 'N/A'}</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Volume</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{formatLargeNumber(stock.volume)}</p>
                </div>

                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Div Yield</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{stock.dividendYield?.toFixed(2) || 'N/A'}%</p>
                </div>
              </div>

              {/* 52 Week Range */}
              <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">52 Week Range</p>
                <div className="flex items-center justify-between text-sm">
                  <span className="font-semibold text-slate-900 dark:text-white">${stock.week52Low?.toFixed(2) || 'N/A'}</span>
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white">${stock.week52High?.toFixed(2) || 'N/A'}</span>
                </div>
              </div>
            </div>
          </div>
