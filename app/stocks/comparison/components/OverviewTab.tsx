  ...
  if (comparedStocks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-gray-600 dark:text-[#a3a3a3]">Add stocks to see the overview</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#171717] rounded-lg p-4 border border-gray-200 dark:border-[#262626]">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-[#fafafa] mb-4">Performance Summary</h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="text-center p-3 bg-gradient-to-br from-green-50 to-green-100 dark:from-green-500/10 dark:to-green-500/5 rounded-lg border border-green-200 dark:border-green-500/20">
            ...
            <div className="text-base font-bold text-green-900 dark:text-[#fafafa] mb-1">{bestPerformer.symbol}</div>
          ...
          <div className="text-center p-3 bg-gradient-to-br from-red-50 to-red-100 dark:from-red-500/10 dark:to-red-500/5 rounded-lg border border-red-200 dark:border-red-500/20">
            ...
            <div className="text-base font-bold text-red-900 dark:text-[#fafafa] mb-1">{worstPerformer.symbol}</div>
          ...
          <div className="text-center p-3 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-500/10 dark:to-blue-500/5 rounded-lg border border-blue-200 dark:border-blue-500/20">
            ...
            <div className="text-base font-bold text-blue-900 dark:text-[#fafafa] mb-1">{highestVolume.symbol}</div>
          ...
          <div className="text-center p-3 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-500/10 dark:to-purple-500/5 rounded-lg border border-purple-200 dark:border-purple-500/20">
            ...
            <div className="text-base font-bold text-purple-900 dark:text-[#fafafa] mb-1">{largestCap.symbol}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {comparedStocks.map((stock) => {
          ...
          return (
            <div key={stock.symbol} className={`bg-white dark:bg-[#171717] rounded-lg p-4 border-2 ${isPositive ? 'border-green-200 dark:border-green-500/30' : 'border-red-200 dark:border-red-500/30'}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-lg font-bold text-gray-900 dark:text-[#fafafa]">{stock.symbol}</h4>
                  <p className="text-xs text-gray-600 dark:text-[#a3a3a3] truncate">{stock.name}</p>
                </div>
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stock.color }} />
              </div>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-[#a3a3a3]">Price</span>
                  <span className="text-lg font-bold text-gray-900 dark:text-[#fafafa]">${stock.price.toFixed(2)}</span>
                </div>
                <div className={`flex justify-between items-center p-2 rounded ${isPositive ? 'bg-green-50 dark:bg-green-500/10' : 'bg-red-50 dark:bg-red-500/10'}`}>
                  <span className="text-sm text-gray-700 dark:text-[#a3a3a3]">Change</span>
                  <span className={`text-base font-bold ${isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    {isPositive ? '+' : ''}{stock.changePercent.toFixed(2)}%
                  </span>
                </div>
                <div className="pt-2 border-t border-gray-200 dark:border-[#262626] space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600 dark:text-[#a3a3a3]">Market Cap</span>
                    <span className="font-medium text-gray-900 dark:text-[#fafafa]">{formatLargeNumber(stock.marketCap)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600 dark:text-[#a3a3a3]">P/E Ratio</span>
                    <span className="font-medium text-gray-900 dark:text-[#fafafa]">{stock.peRatio?.toFixed(2) || '-'}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-600 dark:text-[#a3a3a3]">Sector</span>
                    <span className="font-medium text-gray-900 dark:text-[#fafafa] truncate max-w-[120px]">{stock.sector || '-'}</span>
                  </div>
                </div>
              </div>
            </div>
