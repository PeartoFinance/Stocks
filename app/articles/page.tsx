...
export default function ArticlesPage() {
    ...
    return (
      <main className="p-8 dark:bg-gray-900">
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">Learn investing with concise, practical guides and frameworks.</p>
        </div>
  ...
  return (
    <main className="p-8 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 mb-8">
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-2">
            {categories.map(c => (
              <button key={c} onClick={() => setCategory(c)} className={`px-3 py-1 rounded-full text-sm ${category===c?'bg-blue-600 text-white':'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}>{c}</button>
            ))}
          </div>
          <div className="relative w-full lg:w-80">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
            <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search articles…" className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filtered.map((a, i) => (
          <motion.article key={a.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i*0.05 }} className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all">
            <div className="flex items-center justify-between mb-3">
              <span className="inline-flex items-center gap-2 text-sm text-blue-600"><BookOpen className="h-4 w-4"/>{a.category}</span>
              <span className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1"><Clock className="h-4 w-4" />{a.readTime} min</span>
            </div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">{a.title}</h2>
            <p className="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">{a.summary}</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {a.tags.map(t => <span key={t} className="px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md"><Tag className="h-3 w-3 inline mr-1" />{t}</span>)}
            </div>
            <div className="flex items-center justify-between pt-4 border-t dark:border-gray-700">
