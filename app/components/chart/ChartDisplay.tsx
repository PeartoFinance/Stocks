      ...
      <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-gray-900 dark:bg-slate-900 p-8' : ''}`}>
        ...
        <div className={`${isFullscreen ? 'h-[calc(100vh-120px)]' : 'h-96 md:h-[500px]'} relative`}>
          ...
          {showMovingAverages && movingAveragesData.length > 0 && (
            <div className="absolute top-2 left-2 bg-white dark:bg-slate-800 bg-opacity-90 dark:bg-opacity-90 p-2 rounded text-xs">
              <div className="font-medium mb-1 text-gray-900 dark:text-gray-100">MA Ribbon</div>
              {movingAveragesData.map((ma: any) => (
                <div key={ma.period} className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700 dark:text-slate-300">MA{ma.period}: {ma.data.length > 0 ? formatPrice(ma.data[ma.data.length - 1].value) : 'N/A'}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {showVolumeProfile && volumeProfileData.length > 0 && (
            <div className="absolute top-2 right-12 bg-white dark:bg-slate-800 bg-opacity-90 dark:bg-opacity-90 p-2 rounded text-xs max-w-32">
              <div className="font-medium mb-1 text-gray-900 dark:text-gray-100">Volume Profile</div>
              {volumeProfileData.slice(-5).map((vp: any, i: number) => (
                <div key={i} className="flex justify-between gap-2 text-gray-700 dark:text-slate-300">
          ...
          {showGaps && priceGapsData.length > 0 && (
            <div className="absolute bottom-2 left-2 bg-white dark:bg-slate-800 bg-opacity-90 dark:bg-opacity-90 p-2 rounded text-xs">
              <div className="font-medium mb-1 text-gray-900 dark:text-gray-100">Price Gaps</div>
              {priceGapsData.slice(0, 3).map((gap: any, i: number) => (
                <div key={i} className={`flex items-center gap-1 ${gap.isGapUp ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <span className="text-gray-700 dark:text-slate-300">{gap.gapPercent.toFixed(1)}%</span>
                </div>
              ))}
            </div>
          )}
