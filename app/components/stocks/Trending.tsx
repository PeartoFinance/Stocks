      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 sm:p-6 transition-colors duration-300">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-medium text-slate-900 dark:text-white transition-colors duration-300">Trending Stocks</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 transition-colors duration-300">Most talked about and actively traded stocks</p>
            </div>
          </div>
          <button
            onClick={refreshData}
            ...
            <div className="block lg:hidden overflow-x-auto">
              <table className="w-full min-w-[600px] text-sm">
                <thead className="bg-gradient-to-r from-purple-50 via-violet-50 to-purple-50 dark:from-gray-800 dark:via-gray-800 dark:to-gray-700 border-b-2 border-purple-200 dark:border-gray-600">
                  <tr>
                    <th className="px-3 py-3 text-left text-xs font-medium text-purple-700 dark:text-purple-400 uppercase">Stock</th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-purple-700 dark:text-purple-400 uppercase">Price</th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-purple-700 dark:text-purple-400 uppercase">Change</th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-purple-700 dark:text-purple-400 uppercase">Volume</th>
                    <th className="px-3 py-3 text-right text-xs font-medium text-purple-700 dark:text-purple-400 uppercase">MCap</th>
                  </tr>
                </thead>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-pearto-border text-sm">
                  <thead className="bg-slate-50 dark:bg-slate-800 transition-colors duration-300">
                    <tr>
                      <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase transition-colors duration-300">Trending</th>
                        ...
                        <td className="px-4 py-2 whitespace-nowrap">
                          ...
                          <Link href={`/stock/${stock.symbol.toLowerCase()}`} className="group block">
                            <div className="flex flex-col items-start">
                              <div className="text-sm font-medium text-slate-900 dark:text-white group-hover:text-purple-600 transition-colors leading-tight">
                                {stock.symbol}
                              </div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[160px] leading-tight mt-0.5 transition-colors duration-300">
                                {stock.name}
                              </div>
                            </div>
                          </Link>
