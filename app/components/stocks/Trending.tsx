            ...
            <div className="block lg:hidden">
              ...
              <table className="w-full min-w-[600px] text-sm">
                ...
                <tbody className="bg-white dark:bg-slate-900/95 divide-y divide-slate-100 dark:divide-slate-800">
                  {trendingStocks.map((stock) => stock && (
                    <tr key={stock.symbol} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      <td className="px-3 py-3 whitespace-nowrap">
                        <Link href={`/stock/${stock.symbol?.toLowerCase() || ''}`}>
                          <div className="text-sm font-medium text-slate-900 dark:text-white">{stock.symbol || 'N/A'}</div>
                          <div className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[100px]">{stock.name || 'Unknown'}</div>
                        </Link>
                      </td>
                      <td className="px-3 py-3 text-right text-sm font-medium text-slate-900 dark:text-white">
                        <PriceDisplay amount={stock.price} />
                      </td>
                      <td className={`px-3 py-3 text-right text-sm font-medium ${(stock.change || 0) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {trendingStocks.map((stock, index) => stock && (
                      <motion.tr
                        key={stock.symbol}
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className={`flex items-center justify-end gap-1 font-medium ${stock.change >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                            {stock.change >= 0 ? '+' : ''}<PriceDisplay amount={Math.abs(stock.change || 0)} showSign={false} /> ({stock.change >= 0 ? '+' : ''}{(stock.changePercent || 0).toFixed(2)}%)
                          </div>
                        </td>
