      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div
            ...
            className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 overflow-hidden"
          >
                ...
                <motion.div key={stock.symbol} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4">
                  <Link href={`/stock/${stock.symbol.toLowerCase()}`}>
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500 truncate max-w-[150px]">{stock.name}</span>
                ...
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {['Price', 'Change', 'Volume', 'MarketCap', 'PE'].map((key) => (
                      <th key={key} onClick={() => handleSort(key === 'MarketCap' ? 'marketCap' : key.toLowerCase())} className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredStocks.map((stock) => (
                    <tr key={stock.symbol} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-2 text-sm font-medium text-gray-900">${stock.price.toFixed(2)}</td>
                      <td className={`px-4 py-2 text-sm font-medium ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {stock.changePercent.toFixed(2)}%
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-900">{formatNumber(stock.volume || 0)}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{formatMarketCap(stock.marketCap)}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{stock.peRatio?.toFixed(2) || '—'}</td>
