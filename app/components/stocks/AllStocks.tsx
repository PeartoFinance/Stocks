...
export default function AllStocks({ className = '' }: AllStocksProps) {
  ...
  return (
    <div className={`space-y-4 sm:space-y-6 ${className}`}>
      {/* Search Bar */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-3 sm:p-4 transition-colors duration-300">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-pearto-gray" />
            <input
              type="text"
              placeholder="Search by symbol or company..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 sm:py-2.5 border border-slate-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-pearto-green focus:border-blue-500 dark:focus:border-pearto-green bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-pearto-gray text-sm sm:text-base transition-colors duration-300"
            />
          </div>
          <button
            ...
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 border rounded-lg transition-colors text-sm sm:text-base whitespace-nowrap ${showFilters || hasActiveFilters
              ...
              : 'bg-white -800 border-slate-300 dark:border-gray-600 text-slate-700 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-700'
              }`}
          >
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div
            ...
            className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-3 sm:p-4 lg:p-6 overflow-hidden transition-colors duration-300"
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-medium text-slate-900 dark:text-white transition-colors duration-300">Advanced Filters</h3>
              {hasActiveFilters && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-400 mb-1.5 transition-colors duration-300">Sector</label>
                <select
                  value={filters.sector}
                  onChange={(e) => setFilters({ ...filters, sector: e.target.value })}
                  className="w-full px-3 py-2 border border-slate-300 dark:border-gray-600 rounded-lg text-sm appearance-none bg-white dark:bg-slate-800 transition-colors duration-300"
                >
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-400 mb-1.5 transition-colors duration-300">Exchange</label>
                <select
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-400 mb-1.5 transition-colors duration-300">Min Price</label>
                <input
                  type="number"
                  placeholder="$0"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-400 mb-1.5 transition-colors duration-300">Max Price</label>
                <input
                  type="number"
                  placeholder="$∞"
