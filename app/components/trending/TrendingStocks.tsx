...
export default function TrendingStocks({ className = '' }: TrendingStocksProps) {
  ...
  const getTrendColor = (trendType: TrendingStock['trendType']) => {
    switch (trendType) {
      ...
      default: return 'bg-gray-100 dark:bg-gray-700 text-slate-700 dark:text-gray-400';
    }
  };
        ...
        <main className="p-8">
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              ...
              <h2 className="text-xl font-semibold text-slate-900 dark:text-white transition-colors duration-300">Loading Trending Stocks</h2>
              <p className="text-slate-600 dark:text-gray-400 transition-colors duration-300">Please wait while we fetch the latest trending data...</p>
            </div>
          </div>
        </main>
      </div>
    ...
    <div className={`min-h-screen ${className}`}>
        ...
        <div className="flex flex-col xl:flex-row gap-4 lg:gap-6">
          ...
          <div className="flex-1">
            ...
            >
              ...
              {/* Sector Distribution */}
              <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm border border-gray-100 dark:border-pearto-border-subtle transition-colors duration-300">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-3 transition-colors duration-300">Sector Distribution</h3>
                <div className="h-56">
                  <Doughnut
              ...
              <div className="flex flex-col gap-2 sm:hidden">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="px-2 py-1 text-[10px] border border-slate-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:focus:ring-pearto-green bg-white dark:bg-gray-700 text-slate-900 dark:text-white transition-colors duration-300"
                  >
                  ...
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-7 pr-2 py-1 text-[10px] border border-slate-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 dark:focus:ring-pearto-green bg-white dark:bg-gray-700 text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-pearto-gray transition-colors duration-300"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="hidden sm:flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as any)}
                    className="px-4 py-2 text-sm border border-slate-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-pearto-green bg-white dark:bg-gray-700 text-slate-900 dark:text-white transition-colors duration-300"
                  >
                  ...
                  <div className="relative w-64">
                    ...
                    <input
                      ...
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full pl-10 pr-4 py-2 text-sm border border-slate-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-pearto-green bg-white dark:bg-gray-700 text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-pearto-gray transition-colors duration-300"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[700px]">
                  <thead className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-800 dark:via-gray-800 dark:to-gray-700 border-b-2 border-blue-200 dark:border-gray-600">
                    <tr>
                      <th className="px-3 sm:px-6 py-4 text-left text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Stock</th>
                      <th className="px-3 sm:px-6 py-4 text-right text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Price</th>
                      <th className="px-3 sm:px-6 py-4 text-right text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Change</th>
                      <th className="px-3 sm:px-6 py-4 text-right text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Volume</th>
                      <th className="px-3 sm:px-6 py-4 text-right text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Mkt Cap</th>
                      <th className="px-3 sm:px-6 py-4 text-center text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Score</th>
                    </tr>
                  </thead>
                    ...
                    {filteredStocks.map((stock, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="flex flex-col">
                                <Link
                                  ...
                                  className="text-sm font-bold text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors truncate max-w-[100px] sm:max-w-[150px]"
                                >
                                  {stock.symbol}
                                </Link>
                                <p className="text-xs text-slate-600 dark:text-gray-400 truncate max-w-[100px] sm:max-w-[150px]">{stock.name}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right">
                            <div className="text-sm font-bold text-slate-900 dark:text-white">{formatPrice(stock.price)}</div>
                          </td>
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right">
                            <div className="inline-flex flex-col items-end">
                              <span className={`text-sm font-bold ${stock.change >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                          ...
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right">
                            <div className="text-sm font-bold text-slate-900 dark:text-white">{formatNumber(stock.volume)}</div>
                          </td>
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right">
                            <div className="text-sm font-bold text-slate-900 dark:text-white">{formatNumber(stock.marketCap)}</div>
                          </td>
                          <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-center">
                            <div className="inline-flex items-center px-3 py-1.5 rounded-lg bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
                              <span className="text-sm font-bold text-blue-700 dark:text-blue-400">{stock.trendScore.toFixed(0)}</span>
                            </div>
                          </td>
                        </tr>
                      );
            ...
            {filteredStocks.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-2 transition-colors duration-300">No trending stocks found</h3>
      ...
      <div className={`fixed top-0 right-0 h-full w-96 bg-white dark:bg-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${isAIPanelOpen ? 'translate-x-0' : 'translate-x-full'
        ...
        <div className="h-full flex flex-col">
          {/* AI Panel Header */}
          <div className="px-4 py-3 border-b border-slate-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-purple-50 transition-colors duration-300">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Brain className="h-5 w-5 text-blue-600" />
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white transition-colors duration-300">AI Trend Analysis</h3>
              </div>
              <button
