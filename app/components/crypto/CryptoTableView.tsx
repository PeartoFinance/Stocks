...
export default function CryptoTableView({ cryptoData, loading = false, viewMode }: CryptoTableViewProps) {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 dark:bg-pearto-card rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 dark:border-pearto-border p-8 transition-colors duration-300">
        <div className="flex items-center justify-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-600"></div>
          <span className="ml-3 text-slate-600 dark:text-gray-400 dark:text-pearto-cloud transition-colors duration-300">Loading crypto data...</span>
        </div>
      </div>
    );
  }
 ...
 if (viewMode === 'heatmap') {
        ...
        {cryptoData.map((crypto, index) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  {crypto?.logoUrl ? (
                    <img 
                      src={crypto.logoUrl} 
                      alt="" 
                      className="h-5 w-5 md:h-6 md:w-6 rounded-full bg-white dark:bg-gray-800 dark:bg-pearto-card/10 p-0.5 transition-colors duration-300" 
                    />
                  ) : (
                    <div className="h-5 w-5 md:h-6 md:w-6 rounded-full bg-white dark:bg-gray-800 dark:bg-pearto-card/20 flex items-center justify-center text-[8px] font-bold uppercase transition-colors duration-300">
              ...
              <div className="absolute inset-0 bg-white dark:bg-gray-800 dark:bg-pearto-card opacity-0 group-hover:opacity-5 transition-opacity pointer-events-none" />
            </Link>
          );
        })}
      </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 dark:bg-pearto-card rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 dark:border-pearto-border overflow-hidden transition-colors duration-300">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-pearto-surface dark:to-pearto-card border-b border-slate-200 dark:border-gray-700 transition-colors duration-300">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">#</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">Cryptocurrency</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">Price</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">24h Change</th>
              {/* Hidden on small mobile screens to prevent overflow */}
              <th className="hidden sm:table-cell px-4 py-3 text-right text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">Market Cap</th>
              <th className="hidden md:table-cell px-4 py-3 text-right text-xs font-medium text-slate-700 dark:text-gray-400 dark:text-pearto-cloud uppercase tracking-wider transition-colors duration-300">Volume (24h)</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-pearto-border transition-colors duration-300">
            {cryptoData.map((crypto, index) => (
              <tr 
                key={crypto.id || crypto.symbol || index} 
                className="hover:bg-gray-50 dark:bg-gray-700 dark:bg-gray-700 transition-colors duration-200 cursor-pointer"
onClick={() => handleRowClick(crypto.symbol)}                 >
                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-slate-600 dark:text-gray-400 dark:text-pearto-cloud transition-colors duration-300">
                  {crypto.rank || index + 1}
                </td>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:gap-2">
                      <Link href={`/crypto/${crypto.symbol}`} className="hover:underline transition-colors duration-300">
                        <span className="text-xs md:text-sm font-medium text-slate-900 dark:text-white dark:text-pearto-luna truncate max-w-[70px] md:max-w-[100px] transition-colors duration-300">{crypto.name || 'Unknown'}</span>
                      </Link>
                      <div className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 dark:text-pearto-gray uppercase transition-colors duration-300">{crypto.symbol || '??'}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-right">
                  <div className="text-xs md:text-sm font-medium text-slate-900 dark:text-white dark:text-pearto-luna transition-colors duration-300">{formatPrice(crypto.price)}</div>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-right">
                ...
                <td className="hidden sm:table-cell px-4 py-3 whitespace-nowrap text-right">
                  <div className="text-xs md:text-sm font-medium text-slate-900 dark:text-white dark:text-pearto-luna transition-colors duration-300">{formatMarketCap(crypto.marketCap)}</div>
                </td>
                <td className="hidden md:table-cell px-4 py-3 whitespace-nowrap text-right">
                  <div className="text-xs md:text-sm font-medium text-slate-900 dark:text-white dark:text-pearto-luna transition-colors duration-300">
