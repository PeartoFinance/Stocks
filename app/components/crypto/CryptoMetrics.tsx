...
export default function CryptoMetrics({ globalMetrics, loading = false, onRefresh, refreshing = false }: CryptoMetricsProps) {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white  dark:bg-pearto-card rounded-xl shadow-sm border border-gray-200 dark:border-pearto-border p-6 animate-pulse transition-colors duration-300">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div className="bg-white dark:bg-pearto-card rounded-xl shadow-sm border border-gray-200  dark:border-pearto-border p-6 transition-colors duration-300">
        ...
        <div className="text-2xl font-bold text-gray-900  dark:text-pearto-luna mb-2 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-pearto-card  rounded-xl shadow-sm border border-gray-200 dark:border-pearto-border  p-6 transition-colors duration-300">
        ...
        <div className="text-2xl font-bold text-gray-900 dark:text-pearto-luna dark:text-pearto-lun mb-2 transition-colors duration-300">
          {formatVolume(globalMetrics?.totalVolume24h || 0)}
        </div>
        <div className="text-sm text-gray-500  dark:text-pearto-gray transition-colors duration-300">Last 24 hours</div>
      </div>
      ...
      <div className="bg-white  dark:bg-pearto-card rounded-xl shadow-sm border border-gray-200 dark:border-pearto-border  p-6 transition-colors duration-300">
        ...
        <div className="text-sm text-gray-500 dark:text-pearto-gray  transition-colors duration-300">Market share</div>
      </div>
      ...
      <div className="bg-white  dark:bg-pearto-card rounded-xl shadow-sm border border-gray-200  dark:border-pearto-border p-6 transition-colors duration-300">
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-500  dark:text-pearto-gray transition-colors duration-300">Tracked</div>
          {onRefresh && (
            <button
              onClick={onRefresh}
              disabled={refreshing}
              className="p-2 text-gray-500  dark:text-pearto-gray hover:text-gray-700   transition-colors disabled:opacity-50"
            >
