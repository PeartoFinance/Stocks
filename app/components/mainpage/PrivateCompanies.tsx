    ...
    return (
      <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-8 ${className}`}>
        <div className="flex items-center justify-center">
          ...
          <span className="ml-3 text-slate-600 dark:text-slate-400 transition-colors duration-300">Loading private companies...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-750 transition-colors duration-300">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300 first:rounded-tl-lg last:rounded-tr-lg">Symbol</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300">Name</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300">Price</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300">52-Wk %</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300">Last Funding</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider transition-colors duration-300 first:rounded-tl-lg last:rounded-tr-lg">Valuation</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700 transition-colors duration-300">
            {companies.map((company) => (
              <tr key={company.symbol} className="hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors duration-200">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 bg-gradient-to-br from-purple-100 to-purple-200 dark:from-purple-500/20 dark:to-purple-600/20 rounded-full flex items-center justify-center transition-colors duration-300">
                      <span className="text-xs font-medium text-purple-600 dark:text-purple-400 transition-colors duration-300">
                ...
                <td className="px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 max-w-[150px] truncate transition-colors duration-300">
                  {company.name}
                </td>
                <td className="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white text-right transition-colors duration-300">
                  {formatPrice(company.price)}
                </td>
                <td className={`px-4 py-3 text-sm text-right font-medium transition-colors duration-300 ${company.changePercent >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                  }`}>
                ...
                <td className="px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300 text-right transition-colors duration-300">
