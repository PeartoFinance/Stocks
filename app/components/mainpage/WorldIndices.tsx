...
export default function WorldIndices({ className = '' }: WorldIndicesProps) {
  ...
  const renderTabContent = () => {
    ...
    if (loading) {
      return (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex justify-between items-center p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg animate-pulse transition-colors duration-300">
    ...
    if (data.length === 0) {
      return (
        <div className="flex items-center justify-center h-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 transition-colors duration-300">
  ...
  const renderIndexCard = (index: WorldIndex) => (
    <div key={index.symbol} className="flex justify-between items-center p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm transition-colors duration-300">
      <div>
  ...
  const renderCryptoCard = (crypto: any) => {
    ...
    return (
      <div key={crypto.id || crypto.symbol} className="flex justify-between items-center p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm hover:shadow-md transition-shadow">
        <div className="flex-1">
          {cryptoSymbol ? (
            <Link
            ...
            >
              <div className="text-sm font-medium text-slate-900 dark:text-white group-hover:text-emerald-600  transition-colors duration-300">
                {crypto.name || 'Unknown'}
                <span className="ml-1 text-xs text-slate-400 group-hover:text-emerald-500 transition-colors duration-300">→</span>
              </div>
              <div className="text-xs text-slate-500 dark:text-slate-400 group-hover:text-emerald-500 transition-colors duration-300">{crypto.symbol || 'N/A'}</div>
            </Link>
          ) : (
            <div className="text-left">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-400 transition-colors duration-300">{crypto.symbol || 'N/A'}</div>
            </div>
          )}
        </div>
        <div className="text-right">
  ...
  const renderMovers = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 transition-colors duration-300">
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 ${className}`}>
        ...
        {['US', 'EU', 'ASIA', 'CRYPTO'].map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === tab
                ...
                : 'text-slate-600 dark:text-slate-400 hover:text-emerald-600  hover:bg-emerald-50'
              }`}
          >
            {tab}
          </button>
